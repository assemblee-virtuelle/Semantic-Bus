...
const DfobProcessor = require('@semantic-bus/core/helpers/dfobProcessor');
...
class Engine {
  ...
  async resolveComponent() {
    try {
      const components = await workspace_component_lib.get_all({
        workspaceId: this.originComponent.workspaceId
      });
      this.componentsResolving = components;
      const workflow = await workspace_lib.getWorkspace(this.originComponent.workspaceId);
      ...
      this.owner = user;
      const process = await workspace_lib.createProcess({
      ...
      if (process.state == 'stop') {
        ...
        if (this.owner.credit >= 0 || (config.privateScript && config.privateScript.length == 0) || config.free == true) {
          ...
          if (processingNode != undefined) {
            ...
            const nodesProcessingInputs = this.pathResolution.nodes.filter(npi =>
              npi.targets.map(t =>
                t.target.component._id
              ).includes(processingNode.component._id)
            );

            const module = technicalComponentDirectory[processingNode.component.module];
            ...
            if (componentFlow.dataFlow != undefined && componentFlow.primaryflow == undefined) {
              const err = new Error('primary flow could not be identified');
              ...
              if (componentFlow.deeperFocusData) {
                ...
                try {
                  ...
                  const dfobPathNormalized = stringReplacer.execute(dfobPath, processingNode.queryParams?.queryParams, componentFlow.primaryflow?.data);
                  const dfobTab = dfobPathNormalized.length > 0 ? dfobPathNormalized.split('.') : [];

                  const dfobFragmentFlow = await this.buildDfobFragmentFlow(
                  ...
                  if (dfobFragmentSelected.length == 0) {
                    ...
                    if (!module.workWithFragments) {
                      ...
                      for (const sf of componentFlow.secondaryFlow) {
  ...
  async historicEndAndCredit(processingNode, startTime, frag, error, dfob) {
    ...
    try {
      ...
      const module = processingNode.component.module;
  ...
  buildPathResolution(workspace, component, requestDirection, depth, usableComponents, buildPath, queryParams, buildPathCauseLink) {
    if (depth < 100) {
      ...
      const module = technicalComponentDirectory[component.module];
...
module.exports = {
  execute: function (component, requestDirection, stompClient, callerId, pushData, queryParams, tracerId) {
    const engine = new Engine(component, requestDirection, stompClient, callerId, pushData, queryParams, tracerId);
