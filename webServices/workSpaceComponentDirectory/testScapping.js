...
module.exports = {
  ...
  makeRequest: function (actions, url, flowData, flow_before, fix_url) {
    ...
    function _setValue(action, _page) {
      return new Promise(function (resolve, reject) {
        console.log(" ----- action SET VALUE ----", action.setValue)
        _page.evaluate(function (action) {
          document.querySelector(action.selector).value = action.setValue
                ...
                _waitFor(page, function () {
                  ...
                  _setValue(actions[deeth], page).then(function (res) {                
                    ...
                    _aggregateAction(actions, page, deeth, data, outObj, _ph).then(function (res) {
                      ...
                      let fullError = new Error(err);
                      fullError.displayMessage = "Scrappeur : Erreur lors de votre traitement 6 de page : " + actions[deeth].action;
                      reject(fullError)
                    })
                  })
                }, function (err) {
              ...
              case ("click"):
                _waitFor(page, function () {
                  ...
                  simulateClick(actions[deeth], page, outObj).then(function (resultat) {
                    console.log(" -------- simulateClick click ------ ", resultat)
                    deeth += 1
                    // page.open(res)
                    console.log(page)
                    _aggregateAction(actions, page, deeth, data, outObj, _ph).then(function (res) {
