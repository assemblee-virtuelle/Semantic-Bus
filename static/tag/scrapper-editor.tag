<scrapper-editor>
  ...
  <style> 
    ...
    <!-- this.on('mount', function () {
        this.tags.zentable.on('rowSelect', function (data) {
          this.currentRowId=data.rowid
          this.fieldValueChemin = data.chemin;
          this.update();
        }.bind(this));
        this.tags.zentable.on('addRow', function () {
          this.data.specificData.scrappe.push({chemin: this.fieldValueChemin});
          this.tags.zentable.data = this.data.specificData.scrappe;
        }.bind(this));
      ...
      RiotControl.on('item_current_changed', function (data) {
        ...
        console.log('item_current_changed');
        if (this.tags.zentable != undefined) {
          this.tags.zentable.data = this.data.specificData.scrappe;
        }
        this.update();
    ...
    this.cheminValueChange = function (e) {
      this.fieldValuechemin = e.target.value;
      //console.log("cheminValueChange");
      this.data.specificData.scrappe[this.currentRowId] = {chemin: this.fieldValuechemin};
      //console.log("value change", this.fieldValuechemin);
      this.tags.zentable.data = this.data.specificData.scrappe;
      console.log("zentable change |||||",   this.data.specificData.scrappe);
    }.bind(this); -->
