  ...
  <div id="graphContainer" style="flex-grow:1;" class="containerH contentrGrid">
    <svg ref="graphSvgCanvas" style="flex-grow:1; position: relative">
      ...
      <div class="tableRowStatus" style={showComponent? "right: 30%": "right: 5%"}>
        <div class={innerData && innerData.status ? innerData.status : "Stop"} onClick={stopFlow}>
          <img if={innerData && innerData.status === "running"} src="./image/loading.svg" class="img-status loaderImg" />
          <div class="status-div">
            {innerData && innerData.status !== "running" ? innerData.status : "Stop"}
          </div>
        </div>
      </div>
    </svg>
